...
class customer extends CI_Controller {
	...
	public function index()
	{
        ...
        $this->parser->parse('public/view_index.php', $data);   
    }
	...
	public function sell()
	{
        if ($this->input->server('REQUEST_METHOD') == 'POST'){	
            ...
            if(!$this->input->post('buttonSubmits'))
    		{
                ...
                $this->load->view('public/sell', $cdata);
    		}
            else{
                ...
                $this->form_validation->set_rules('s_price', 'Selling Price', 'numeric|greater_than[1]');
                $this->form_validation->set_rules('w_end', 'Warranty End date');
				$this->form_validation->set_rules('v_id', 'Vehicle Id', 'required');
                
                if ($this->form_validation->run() == FALSE) {
                    $data['vRow'] = $this->model_vehicle->get($cid);
                    $this->load->view('public/sell', $data);
                }
                else {
